...
export default function PostCard({ post }: { post: Post }) {
  ...
  return (
    <div className="group relative flex flex-col md:flex-row gap-5 border-b-2 border-[rgba(255,255,255,0.1)] py-4 pb-10 last-of-type:border-b-0 first-of-type:border-t-0">
        ...
        <div className="flex-1 flex flex-col space-y-6">
          <div className="space-y-4 flex-1">
            <a href={`/blog/${post.slug}`} target="_blank" rel="noopener noreferrer">
              ...
              <h3 className="text-xl font-semibold leading-snug group-hover:underline group-hover:decoration-[rgba(255,255,255,0.25)]">
                {post.title}
              </h3>
            </a>
            {post.excerpt && (
      ...
      <div className="flex-1 xl:max-w-[500px] order-1 md:order-2">
        <div className="w-full h-64  overflow-hidden bg-[#0c0c0c]">
          <a href={`/blog/${post.slug}`} target="_blank" rel="noopener noreferrer">
            <img src={imageSrc} onError={() => setImgError(true)} alt={post.title}
